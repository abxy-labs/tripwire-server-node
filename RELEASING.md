...
2. Ensure the `@abxy` npm scope is owned and usable for publishing `@abxy/foil-server`.
3. For the first release only, add a short-lived granular `NPM_PUBLISH_TOKEN` GitHub Actions secret with publish access to `@abxy/foil-server`.
